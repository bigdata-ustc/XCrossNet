...
class BaseModel(nn.Module):
            ...
            try:
                ...
                with tqdm(enumerate(train_loader), disable=verbose != 1) as t:
                    for index, (x_train, y_train) in t:
                        ...
                        optim.zero_grad()
                        total_loss = loss_func(y_pred, y.squeeze(), reduction='sum')
                        ...
                        epoch += 1
                        if epoch % 1000 == 0 and write_file is not None:
                            with open(write_file, 'a') as f:
                                f.writelines('step: '+str(epoch)+';loss: '+str(total_loss.item())+' (time: '
    ...
    def compile(self, optimizer,
        ...
        """
        :param weight_decay: l2 reg
        :param optimizer: String (name of optimizer) or optimizer instance. See [optimizers](https://pytorch.org/docs/stable/optim.html).
        :param loss: String (name of objective function) or objective function. See [losses](https://pytorch.org/docs/stable/nn.functional.html#loss-functions).
        :param metrics: List of metrics to be evaluated by the model during training and testing. Typically you will use `metrics=['accuracy']`.
        """

        self.optim = self._get_optim(optimizer, weight_decay)
        self.loss_func = self._get_loss_func(loss)
        self.metrics = self._get_metrics(metrics)

    def _get_optim(self, optimizer, weight_decay):
        if isinstance(optimizer, str):
            if optimizer == "sgd":
                optim = torch.optim.SGD(self.parameters(), lr=0.01, weight_decay=weight_decay)
            elif optimizer == "adam":
                optim = torch.optim.Adam(self.parameters(), weight_decay=weight_decay)  # 0.001
            elif optimizer == "adagrad":
                optim = torch.optim.Adagrad(self.parameters(), weight_decay=weight_decay)  # 0.01
            elif optimizer == "rmsprop":
                optim = torch.optim.RMSprop(self.parameters(), weight_decay=weight_decay)
            else:
